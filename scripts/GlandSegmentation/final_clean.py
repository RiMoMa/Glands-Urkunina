...
def main():
    ...
    detections = [f for f in os.listdir(detections_path_xml) if f.endswith("_completed.xml")]
    ...
    for detection_file in detections:
        ...
        output_xml_path = os.path.join(detections_path_xml, f"{case_name}_corrected_final.xml")

        if os.path.exists(svs_path) and os.path.exists(xml_path):
            print(f"Processing {case_name}...")
            if os.path.exists(output_xml_path):
                print(f"El archivo ya existe en {output_xml_path}")
                continue

            clean_binary_mask_with_annotations(svs_path, xml_path, output_xml_path,kernel_size=1)
        else:
