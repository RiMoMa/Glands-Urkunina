...
def plot_labels_color_op(label_im, cmap='tab20c'):
    """
    Helper function to visualize the gland-level masking.
    This loops through the color map (cmap) defined in matplotlib.
    @param label_im: 2D label image. Each unique value represents a unique label.
    @param cmap: cmap defined in matplotlib documentation
    @return: gland-level segmentation with color based on cmap
    """
    # Construct a color image to superimpose
    h, w = label_im.shape
    color_mask = np.zeros((h, w, 3))

    get_cmap = plt.cm.get_cmap(cmap)

    # Loop through the cmap for each color with its associated labels
    for i in range(np.max(label_im)):
        color_mask[label_im == i + 1] = list(get_cmap(i))[:-1]
